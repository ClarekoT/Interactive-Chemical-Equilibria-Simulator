...
class ChemicalSystem:
    """
    The central engine that models the state of a chemical system. It holds the species, the reactions, and the current physical conditions (moles, V, T).
    ...
    def run_simulation(self, t_end: Optional[float] = None, rate_tolerance: float=1e-7, max_iterations: int=75) -> None:
        ...
        if t_end is not None: # simulation runs for a fixed duration if t_end is provided
            ...
            solution = solve_ivp(
                ...
                method=self.method, rtol=self.rtol, atol=self.atol, max_step=t_end / 100.0)
    ...
    def calculate_thermodynamics(self, fwd_idx: int, rev_idx: int, external_results: Optional[dict]=None) -> Optional[Dict[str, np.ndarray]]:
        ...
        Kc_kinetic = np.divide(k_f, k_r, out=np.zeros_like(k_f), where=k_r!=0)

        correction_factor = np.ones_like(T_arr)
        solvent_names = {s.name for s in self.species_list if s.phase == 'solvent'}

        # if solvent is a reactant (e.g., HA + H2O -> ...), it appears in the rate law denominator.
        # to cancel it out (make it 1), Kc_kinetic is multiplied by [solvent].
        for r_name, coeff in r_fwd.reactants.items():
            if r_name in solvent_names:
                # get solvent concentration (n/V)
                # note: solvent moles are in species_data, usually constant.
                conc_solvent = species_data[r_name] / V_arr
                correction_factor *= (conc_solvent ** coeff)

        # if solvent is a product, we divide.
        for p_name, coeff in r_fwd.products.items():
            if p_name in solvent_names:
                conc_solvent = species_data[p_name] / V_arr
                correction_factor /= (conc_solvent ** coeff)
                
        Kc = Kc_kinetic * correction_factor
...
class PerturbationSimulation:
    ...
    Handles volume ramps, temperature ramps, and species injections generically and simultaneously."""
    
    def __init__(self, baseline_system: ChemicalSystem, t_start: float, t_end: float, 
    ...
    def _ode_adapter_perturbation(self, t: float, y: np.ndarray) -> np.ndarray:
        ...
        for species, rate in self.injection_rates.items():
            if species in dndt_dict and species not in self.baseline_system.pool_species_names:
                current_n = current_moles.get(species, 0.0)
                dndt_dict[species] += rate 

        # DEBUG
        final_dndt_array = self.baseline_system._state_to_array(dndt_dict)
        if not np.isfinite(final_dndt_array).all():
            logger.critical(f"!!! Non-finite value detected at t={t:.4e} !!!")
            logger.debug(f"    current_moles: {current_moles}")
            logger.debug(f"    current_V: {current_V:.4f}, current_T: {current_T:.2f}")
            logger.debug(f"    dndt after injection:  {dndt_dict}")
            logger.debug(f"    Final dndt array: {final_dndt_array}")
    ...
    def run_perturbation(self) -> None:
        ...
        logger.info("run_perturbation called. Preparing for Stage 1 (Baseline).")
        # check for invalid numbers in the initial baseline data
        y0_check = self.baseline_system._state_to_array(self.baseline_system.results['final_moles'])
        if not np.isfinite(y0_check).all():
            logger.critical("CRITICAL ERROR: Perturbation started with non-finite values from baseline system.")
            logger.error(f"Baseline final_moles: {self.baseline_system.results['final_moles']}")
            self.results = None # ensure simulation fails gracefully
            return # abort
        
        # first stage: history/baseline
        ...
        # seocond stage: applying the stress
        logger.info("Starting Stage 2.")
        t_span = (self.t_start, self.t_end)
        sol = solve_ivp(self._ode_adapter_perturbation, t_span, y0_stage2, dense_output=True, 
                        method=self.baseline_system.method, rtol=self.baseline_system.rtol, atol=self.baseline_system.atol, max_step=(self.t_end - self.t_start) / 50.0)
        logger.info("Stage 2 solve_ivp call finished.")

        if not sol.success and sol.status == -1:
            print(f"ERROR: Solver crashed during perturbation. Reason: {sol.message}")
            return
        ...
        y0_stage3_dict = {s: arr[-1] for s, arr in s2_data.items()}
        final_V = self.target_V
        final_T = self.target_T

        new_sys = ChemicalSystem(
            self.baseline_system.species_list, self.baseline_system.reactions,
            y0_stage3_dict, final_V, final_T, method=self.baseline_system.method, 
            rtol=self.baseline_system.rtol, atol=self.baseline_system.atol, 
            overall_reaction=self.baseline_system.overall_reaction, system_type=self.baseline_system.system_type)
        
        if self.t_global_end is not None and self.t_global_end > self.t_end:
            new_sys.run_simulation(self.t_global_end - self.t_end) 
        else:
        ...
        for s in self.baseline_system.species_order:
            raw_species[s] = np.concatenate([s1_data[s][:-1], s2_data[s][:-1], s3_data[s]])
        ...
        if len(t_sorted) > 1:
            keep_mask = np.concatenate(([True], np.diff(t_sorted) > 1e-12))
        else:
        ...
        n_tot = sum(final_species.values())
        final_P_ideal = (n_tot * R_ATM_L * final_temp) / final_volume

        if self.t_global_end is not None:
