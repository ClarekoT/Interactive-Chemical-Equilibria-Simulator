...
def analyse_acid_base(results: dict, perturbation_window: Optional[tuple] = None) -> Optional[Dict]:
    """Analyses acid-base titration results to find one or more equivalence points."""

    if 'species_data' not in results or perturbation_window is None:
        return {'eq_points': []}

    proton_key = 'H3O+' if 'H3O+' in results['species_data'] else 'H+'
    if proton_key not in results['species_data']:
        return {'eq_points': []}

    t_start, t_end = perturbation_window
    time_arr = results['time']
    window_mask = (time_arr >= t_start) & (time_arr <= t_end)

    if np.sum(window_mask) < 10: # need a reasonable number of points
        return {'eq_points': []}

    t_window = time_arr[window_mask]
    
    vol_arr = results.get('volume', np.full_like(time_arr, results['info']['V']))
    conc_proton_window = results['species_data'][proton_key][window_mask] / vol_arr[window_mask]
    
    safe_protons = np.maximum(conc_proton_window, 1e-16)
    ph_window = -np.log10(safe_protons)

    # create a smooth, continuous function from the discrete data points
    cs_ph = CubicSpline(t_window, ph_window)

    # find approximate locations of equivalence points by finding peaks in the gradient
    t_grid = np.linspace(t_window[0], t_window[-1], num=2000)
    ph_gradient = cs_ph(t_grid, 1) # first derivative (the slope)

    # use a peak finder to locate all potential equivalence points.
    peaks, _ = find_peaks(ph_gradient, prominence=0.1) # prominence=0.1, peak must stand out from its surroundings by at lesat 0.1 pH units/s

    if len(peaks) == 0:
        return {'eq_points': []}

    # for each approximate peak, use a root-finder to pinpoint the exact inflection point
    eq_points = []
    second_deriv = cs_ph.derivative(2) # get the second derivative function

    for peak_idx in peaks:
        t_approx = t_grid[peak_idx]
        search_width = (t_grid[1] - t_grid[0]) * 10 # search +/- 10 grid points
        t_low = max(t_grid[0], t_approx - search_width)
        t_high = min(t_grid[-1], t_approx + search_width)
        
        try:
            # find the exact time 't' where second_deriv(t) = 0
            t_eq = brentq(second_deriv, a=t_low, b=t_high)
            ph_eq = cs_ph(t_eq) # get the pH at this exact time
            
            eq_points.append({'time': t_eq, 'ph': ph_eq})
        except ValueError: # this happens if the second derivative doesn't actually cross 0 within the search interval (safely ignore this case)
            continue
            
    return {'eq_points': eq_points}
