...
def generate_table(current_data: dict, system_object: ChemicalSystem, previous_data: Optional[dict]=None, perturbation_window: Optional[Tuple[float, float]]=None) -> None:
    ...
    metric_labels = [r'Volume ($dm^3$)', 'Temperature (K)', 'Time (s)', 'P(Real) / atm', 'P(Ideal) / atm']
    for name in s_names:
        metric_labels.append(f'[{name}] init')
        metric_labels.append(f'[{name}] final')

    def get_vals(data):
        # handle cases where volume might be an array or a scalar in 'info'
    ...
    if previous_data:
        table_dict['Previous Run'] = get_vals(previous_data)
...
def generate_plot(current_data: dict, system_object: ChemicalSystem, previous_data: Optional[dict]=None, perturbation_window: Optional[Tuple[float, float]]=None,
                            view_window: Optional[Tuple[float, float]]=None, log_x: bool=False, log_y: bool=False,
    ...
    def draw_layer(ax_c: plt.Axes, ax_p: plt.Axes, ax_th: plt.Axes, t: np.ndarray, c_dict: dict, # drawing helper
                   p_arr: np.ndarray, thermo_data: Optional[dict]=None, style: str='-', alpha: float=1.0) -> None:
        # conc & rates
        for n in s_names:
    ...
    draw_layer(ax_conc, ax_pressure_main, ax_thermo_main, t_curr, conc_curr, p_curr, curr_thermo)
    if prev_plot_data:
        draw_layer(ax_conc, ax_pressure_main, ax_thermo_main, prev_plot_data['time'], prev_plot_data['conc'], 
                   prev_plot_data['p_real'], style='--', alpha=0.4)
    ...
    if perturbation_window:
        ...
        draw_layer(ax_conc_detail, ax_pressure_zoom, None, t_curr, conc_curr, p_curr, curr_thermo)
        if prev_plot_data:
            draw_layer(ax_conc_detail, ax_pressure_zoom, None, prev_plot_data['time'], prev_plot_data['conc'], prev_plot_data['p_real'], style='--', alpha=0.4)
        ...
        if curr_thermo: 
            ...
            if curr_thermo.get('Qp') is not None:
                ...
                ax_th_zoom.plot(t_curr, curr_thermo['Kp'], color='deeppink', linestyle=':', alpha=0.6, lw=1.5, label=r'Ideal $K_p$')
    ...
    if previous_data: l1.append(Line2D([0], [0], color='gray', linestyle='--', label='Previous'))
...
def create_interface(system):
    ...
    btn_vol = widgets.Button(description="Change Volume", disabled=True, layout=layout_half)
    btn_temp = widgets.Button(description="Change Temperature", disabled=True, layout=layout_half)
    btn_inj = widgets.Button(description="Inject Species", disabled=True, layout=layout_full)
    ...
    def update_controls(sys):
        ...
        btn_vol.disabled = False; btn_temp.disabled = False; btn_inj.disabled = False
    ...
    def refresh_plot(perturb_window=None, highlight_point=None, fixed_t_end: Optional[float]=None):  
        with output:
            ...
            previous_results = gui_state.prev.results if gui_state.prev else None
            ...
            generate_table(current_results, system_object=gui_state.curr, # generate table
                           previous_data=previous_results, perturbation_window=perturbation_window)
            generate_plot(current_results, system_object=gui_state.curr, previous_data=previous_results,  # generate plot
                            perturbation_window=perturbation_window, view_window=view_window,
                            log_x=chk_log_x.value, log_y=chk_log_y.value, title=txt_title.value, highlight_point=highlight_point,
    ...
    def on_run(b):
        ...
        new_sys = ChemicalSystem(system.species_list, system.reactions, moles, vol_slider.value, temp_slider.value,
                                 method=system.method, rtol=system.rtol, atol=system.atol)
    ...
    def on_perturb(ptype):
        system_to_perturb = st.curr
        if not system_to_perturb: return
        ...
        if t_start >= t_end: # validation to prevent t_start > t_end
            with output:
                print(f"Error: Start time ({t_start:.2e}s) must be less than end time ({t_end:.2e}s).")
            return
        ...
        args = {'baseline_system': system_to_perturb, 't_start': t_start, 't_end': t_end, 't_global_end': t_global_end_param}
        
        # add specific arguments based on the perturbation type
        if ptype == 'volume':
            args.update(perturbation_type='volume', new_V=vol_slider.value)
        elif ptype == 'temperature':
            args.update(perturbation_type='temperature', new_T=temp_slider.value)
        elif ptype == 'injection':
            rates = {name: widget.value for name, widget in injection_widgets.items()}
            args.update(perturbation_type='injection', injection_rates=rates)
            
        perturbation_sim = PerturbationSimulation(**args)
        with output:
        ...
        post_perturb_system = ChemicalSystem(
            ...
            final_moles, final_V, final_T, method=system.method, rtol=system.rtol, atol=system.atol)
    ...
    btn_vol.on_click(lambda b: on_perturb('volume'))
    btn_temp.on_click(lambda b: on_perturb('temperature'))
    btn_inj.on_click(lambda b: on_perturb('injection'))
    ...
    view_controls = widgets.VBox([widgets.HTML("<b style='font-size:14px'>Graph View</b>"),
        widgets.HBox([view_slider, btn_view_update,], layout=widgets.Layout(align_items='center', gap='10px')),
        widgets.HBox([chk_log_x,chk_log_y], layout=widgets.Layout(gap='20px')), y_scale_controls, visibility_controls],
    ...
    display(widgets.VBox([view_controls, output, controls]))